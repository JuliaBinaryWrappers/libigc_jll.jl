...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import libigc_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("libigc_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "libigc_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"libigc", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "libigc_jll", "src", "libigc_jll.jl"))
        @info("libigc_ll dev'ed out to /depot/dev/libigc_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("94295238-5935-5bd7-bb0f-b00942e9bdd5"))
platforms = [Pkg.Artifacts.unpack_platform(e, "libigc", artifacts_toml) for e in artifacts["libigc"]]
...
if best_platform === nothing
    @debug("Unable to load libigc; unsupported platform $(triplet(platform_key_abi()))")
else
