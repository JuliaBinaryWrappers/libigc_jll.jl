# Autogenerated wrapper script for libigc_jll for x86_64-linux-musl-cxx03
export libopencl_clang, iga, libigc, GenX_IR, libiga, libigdfcl
...
const libopencl_clang_splitpath = ["lib64", "libopencl-clang.so.10"]
...
const libigc_splitpath = ["lib64", "libigc.so"]
...
const libiga_splitpath = ["lib64", "libiga64.so"]
...
# This must be `const` so that we can use it with `ccall()`
const libiga = "libiga64.so.1"


# Relative path to `libigdfcl`
const libigdfcl_splitpath = ["lib64", "libigdfcl.so"]
